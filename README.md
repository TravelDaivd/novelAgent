...
多轮对话状态管理：通过HumanMessage、AIMessage、ToolMessage三种消息类型配对约束，确保上下文不丢失、不混淆
...
###  自训练三个垂直领域NLP模型，构建可迁移的领域知识转化能力

在小样本数据起步的条件下，通过LoRA调参、Bert学习率，分层学习率、数据飞轮迭代，独立训练出实体识别、关系抽取、文本分类三个模型，将非结构化文本转化为结构化知识。标注数据从50条扩展至300+条，精度达91.62%。这套模型训练方法论不依赖大规模标注数据，也不绑定小说场景，可直接迁移至医疗、法律、企业内部文档等任意垂直领域。
...
更改 docker.env 中的 “APP_llm__DEEPSEEK_API_KEY” 跟换成你的API_KEY 
### 模式1：在线演示模式（面试官体验推荐）
...
docker-compose up -d
#### 3. 访问前端问答页面：http://127.0.0.1:3000
### 模式2：离线私有化部署（客户内网无外网场景）
